...
if [ -d /etc/soga ]; then
    ...
    case $reinstall_option in
        ...
        2)
            ...
            ;;
        *)
            echo -e "${yellow}无效的选项，跳过安装 soga${plain}"
            ;;
    esac
fi
...
if [ ! -d /etc/soga ]; then
    echo -e "${green_light}/etc/soga 文件夹不存在，开始安装 soga${plain}"
...
echo -e "${yellow}1. Soga配置${plain}"
echo -e "${yellow}2. 解锁配置${plain}"
echo -e "${yellow}3. 审计配置${plain}"
...
case $function_number in
    1)
        echo -e "${yellow}Soga配置${plain}"
        # Soga配置的具体操作
        ;;
    2)
        echo -e "${yellow}解锁配置${plain}"
    ...
    3)
        ...
        case $audit_option in
            ...
            2)
                ...
                echo -e "${yellow}blockList 替换成功${plain}"
                ;;
            *)
                echo -e "${yellow}无效的选项${plain}"
                ;;
        esac
        ;;
    *)
        echo -e "${yellow}无效的操作编号${plain}"
        ;;
esac
