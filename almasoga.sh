...
while true; do
    ...
    case $function_number in
        1)
            ...
            read -p "$(echo -e "${yellow}请选择操作类型：[1. 文件配置 / 2. 修改配置]: ${plain}")" config_option
        ...
        2)
            ...
            read -p "$(echo -e "${yellow}请选择解锁配置的类型：[1. DNS解锁 / 2. DNS修改]: ${plain}")" unlock_option
