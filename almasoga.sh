...
while true; do
    ...
    if [ -d /etc/soga ]; then
        ...
        case $function_number in
            1)
                # Call the function for Soga配置
                soga_configuration
                ;;
            2)
                # Call the function for 解锁配置
                unlock_configuration
                ;;
            3)
                # Call the function for 审计配置
                audit_configuration
                ;;
            4)
                # Call the function for Soga重装
                reinstall_soga
                ;;
            5)
                # Call the function for 流媒体测试
                test_streaming
                ;;
            0)
...
function soga_configuration {
    ...
    case $config_option in
        1)
            # Call the function for 下载配置文件
            download_config_file
            ;;
        2)
            # Call the function for 修改配置文件
            modify_config_file
            ;;
        *)
...
# Function for 解锁配置
function unlock_configuration {
    echo -e "${green}${blink}解锁配置${plain}"

    # Prompt the user to choose the type of unlock configuration
    read -p "$(echo -e "${yellow}${blink}请选择解锁配置的类型：${plain} [1. DNS解锁 / 2. DNS修改]: ")" unlock_option

    case $unlock_option in
        1)
            # Call the function for DNS解锁
            dns_unlock
            ;;
        2)
            # Call the function for DNS修改
            modify_dns
            ;;
        *)
            echo -e "${red}${blink}无效的选项${plain}"
...
function dns_unlock {
    ...
    # Prompt for "region abbreviation", for example, entering "hk" will modify sin.core to hkg.core in /etc/soga's dns.yml
    ...
    case $region_abbr in
        ...
        us)
            ...
            ;;
        *)
            echo -e "${red}${blink}无效的地区缩写${plain}"
            exit 1
            ;;
    esac
...
function modify_dns {
    ...
    case $region_abbr in
        ...
        us)
            ...
            ;;
        *)
            echo -e "${red}${blink}无效的地区缩写: $region_abbr${plain}"
            exit 1
            ;;
    esac
...
function audit_configuration {
    ...
    case $audit_option in
        1)
            # Call the function for 删除审计
            delete_audit
            ;;
        2)
            # Call the function for 增加审计
            add_audit
            ;;
        *)
...
function delete_audit {
    ...
    # Delete the content of /etc/soga/blockList
